...
require "logstash/filters/geoasn"
...
describe LogStash::Filters::GeoASN do
  ...
  describe "ASN db" do
    ...
    sample("ip" => "1.1.1.1") do
      insist { subject["geoasn"]["asn"] } == "Google Inc."
    end
  ...
  describe "defaults" do
    ...
    sample("ip" => "8.8.8.8") do
      ...
      expected_fields.each do |f|
        insist { subject["geoasn"] }.include?(f)
      end
    end

    sample("ip" => "127.0.0.1") do
      # assume geoasn fails on localhost lookups
      reject { subject }.include?("geoasn")
    end
  end
  ...
  describe "correct encodings with default db" do
    ...
    sample("ip" => "1.1.1.1") do
      checked = 0
      expected_fields.each do |f|
        next unless subject["geoasn"][f]
        checked += 1
        insist { subject["geoasn"][f].encoding } == Encoding::UTF_8
      end
      insist { checked } > 0
    end
  ...
  describe "correct encodings with ASN db" do
    ...
    sample("ip" => "1.1.1.1") do
      insist { subject["geoasn"]["asn"].encoding } == Encoding::UTF_8
    end
  ...
  describe "location field" do
    shared_examples_for "an event with a [geoasn][location] field" do
      subject(:event) { LogStash::Event.new("message" => "8.8.8.8") }
      let(:plugin) { LogStash::Filters::GeoASN.new("source" => "message", "fields" => ["country_name", "location", "longitude"]) }
      ...
      it "should have a location field" do
        expect(event["[geoasn][location]"]).not_to(be_nil)
      end
    end

    context "when latitude field is excluded" do
      let(:fields) { ["country_name", "location", "longitude"] }
      it_behaves_like "an event with a [geoasn][location] field"
    end
  ...
  describe "an invalid IP" do
    ...
    describe "filter method outcomes" do
      let(:plugin) { LogStash::Filters::GeoASN.new("source" => "message", "add_tag" => "done", "database" => ASNDB) }
    ...
    context "should return the correct source field in the logging message" do
      sample("ip" => "-") do
        expect(LogStash::Filters::GeoASN.logger).to receive(:error).with(anything, include(:field => "ip"))
        subject
      end
    end
  ...
  describe "returned object identities" do
    let(:plugin) { LogStash::Filters::GeoASN.new("source" => "message") }
    ...
    it "should dup the objects" do
      ...
      event["geoasn"].each do |k,v|
        alt_v = alt_event["geoasn"][k]
