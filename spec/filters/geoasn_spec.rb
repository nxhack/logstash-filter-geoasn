...
describe LogStash::Filters::GeoASN do
  ...
  describe "an invalid IP" do
    ...
    describe "filter method outcomes" do
      let(:plugin) { LogStash::Filters::GeoASN.new("source" => "message", "add_tag" => "done", "database" => CITYDB) }
      ...
      context "when the bad IP is N/A" do
        ...
        it "should add failure tags" do
          expect(event["tags"]).to include("_geoasn_lookup_failure")
        end
      end
      ...
      context "when IP is IPv6 format" do
        ...
        it "should set the target field to an empty hash" do
          expect(event["geoasn"]).not_to be_empty
          expect(event["geoasn"]["city_name"]).not_to be_nil
        end
      end
